...
`r insert_calcZ_exercise(2.1, "H2KG3", "Exercises/Fun/pattern-book-zeros.Rmd")`

----------

`r insert_calcZ_exercise("2.2", "Pdt9jy", "Exercises/Fun/lobster-ride-glasses.Rmd")`

---------

`r insert_calcZ_exercise(2.3, "VIW7T","Exercises/Fun/pattern-book-axis-crossing.Rmd")`

---------

`r insert_calcZ_exercise("2.4", "Qj0JAr", "Exercises/Fun/bear-lay-plant.Rmd")`


<!-- An .observation about giving names to functions that don't have them in standard notation: square, recip, power. Exercise: write square(x) in terms of power. I'm not asking you to square a number, but to create a function which, at any time you want, will square any number you give it as input. -->

::: {.observation}
Functions like `sin()`, `log()`, `pnorm()` are always referred to by name with parentheses enclosing the arguments. Other functions, like $e^x$ or $1/x$ are written in the traditional arithmetic notation, with no name applying.

It's perfectly feasible to arrange so every pattern-book function uses the name/parentheses style. For instance, here are definitions for four of the pattern-book functions that we haven't written in name/parentheses style:
...
We're using different names for the inputs in these three functions just to emphasize that for functions with one input, the name has no significance. You just have to make sure to use the same name on the left- and right-hand sides of the tilde expression. 

The function called `constant()` necessarily has the property that the output does **not depend** on the input. That's why it's constant!

In R, a statement like `x^2` is really just shorthand for using a name/parenthesis style function whose name is `` `^` ``. Or, consider $-3$. In R you would write this `-3` but internally R is applying a function named `` `-` ``, like this: `` `-`(3)``. 
