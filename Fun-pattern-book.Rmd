...
1. Translate the real-world situation into a mathematical form. This translation is called ***mathematical modeling***. For instance, Newton was trying to make sense of the motion of planets in light of his novel theory that gravity is a universal force of attraction between masses, rather than something special to Earth. He modeled force as a quantity that can be measured, simplified the planets to mere masses (another measurable quantity), and supposed that ***position*** and motion were related in a particular way: ***velocity*** being the rate of change of position with respect to time, and ***acceleration*** being the rate of change of velocity with respect to time. His theory of the motion of planets was based on two fundamental mathematical models: First
...
I've written these here in a traditional formula notation so that you can see the connections to the math you've already studied. I've used $x$ to stand for the single input to these functions just because that's traditional.  `r mark(240)`
...
By the end of Block 1, you should be able to list all nine basic pattern-book functions and say which of these features are relevant to each. `r mark(250)`
:::
...
- `~ x` is the part of the R/mosaic notation that says, "The name of the input will be `x`." In traditional notation, the name of the input is in parentheses following the name of the function $g\,{\mathbf{(x)}}$.
- The formula and the name of the input are always related in the R/mosaic notation, part of what's called a ***tilde expression***. The formula is on the left-hand side of the tilde (<span style="font-size: 24pt;"`~`</span>) and the argument name is on the right-hand side of the tilde. If there is more than one argument, all the names go on the right-hand side of the tilde expression, separated by `&`. (Some people prefer to use `+` as the separator. That's fine.)
- `makeFun()` is the R/mosaic operator that takes the information contained in the tilde expression and turns it into an R function.
:::
...
slice_plot(dnorm(x) ~ x, domain(x=c(-3,3))) %>%
  gf_labs(title="Gaussian (also called \"hump\")")
...
Three of the pattern-book functions---$1/x$, $x$, $x^2$---belong to an infinite family called the ***power-law functions***.  Some other examples of power-law functions are $x^3, x^4, \ldots$ as well as $x^{1/2}$ (also written $\sqrt{x}$), $x^{1.36}$, and so on. All of these functions can be written $x^p$, where $x$ is the input and $p$ is a number. `r mark(290)`

Even within the power-law family, it's helpful to consider different but overlapping groups:

1. The ***monomials***: $m_0(x) \equiv x^0$, $m_1(x) \equiv x^1$, $m_2(x) \equiv x^2$, $\ldots$. Of course, $m_0()$ is exactly the same as the constant function since $x^0 = 1$. Likewise, $m_1(x)$ is the same as the identity function since $x^1 = x$. As for the rest, they have just two general shapes: both arms up (for even powers); one arm up and the other down (for odd powers.)
...
`r insert_calcZ_exercise(2.2, "H2KG3", "Exercises/Fun/pattern-book-zeros.Rmd")`

----------

`r insert_calcZ_exercise("2.4", "Pdt9jy", "Exercises/Fun/lobster-ride-glasses.Rmd")`

---------

`r insert_calcZ_exercise(2.5, "VIW7T","Exercises/Fun/pattern-book-axis-crossing.Rmd")`

---------

`r insert_calcZ_exercise("2.6", "Qj0JAr", "Exercises/Fun/bear-lay-plant.Rmd")`

## Instructor notes

It would be a huge coincidence if one of the pattern-book functions was a match for a particular relationship, such as one shown in data. We'll need to tailor the pattern-book functions and combine them in various ways to capture a real-world relationship. `r mark(295)`
...
To illustrate, here's a graph of month-to-month utility data from a house in Minnesota plotted along with the pattern-book function $x$.
