...
So if you know $g()$, you know ${\cal D}_t g(t)$. By making $h$ very small, the "average" starts to look like "instantaneous." Over a small interval of length $h$, every smooth function looks like a straight line and so the average rate of change is very close to the instantaneous rate of change.
...
How can we know that these formulas are right? There are two answers: 

1. We can confirm them by doing the calculations involved in $\lim_{h\rightarrow 0}$. 
2. We can support them by noting that these same formulas appear in any calculus text you choose to use.
3. We can further support them by comparing them to the functions computed from the ${\cal D}_x$ formula when $h$ is very small.

Only (1) is hard. (2) might be doubted by conspiracy theorists. Let's work with (3) to begin.
