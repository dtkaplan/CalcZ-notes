...
```{r penguin-flipper2,echo=FALSE, warning=FALSE, out.width="50%", fig.show="keep", fig.cap="The penguin data doesn't directly describe any sensible mathematical function. (left) We use modeling to create a function that stays close, but not too close, to the data points. (right)"}
...
```{exercise, name="drawing"}
```
<details>`r MC_counter$reset()` `r knitr::knit_child(exercise_file("03", "drawing.Rmd"))`</details>


```{exercise, daylength, name="length-of-day"}
```
<details>`r MC_counter$reset()` `r knitr::knit_child("Exercises/Fun/length-of-day.Rmd")`</details>
