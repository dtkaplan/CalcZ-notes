...
```
<details>`r MC_counter$reset()` `r knitr::knit_child("Exercises/Diff/kid-mean-table.Rmd")`</details>
...
```{r stop-and-go2, echo=FALSE, fig.cap="The position of an imagined car over an hour of time. (black) The red line shows what the position would have been if the car had travelled steadily at the average speed for the hour."}
...
2. Given a function $\partial_t\,(t)$, find the $f(t)$ of which $\partial_t\,f(t)$ is the instantaneous rate of change. This process of finding $f()$ given $\partial_t\,f()$ is called ***anti-differentiation***.
...
```{exercise, name="pine-lead-car"}
```
<details>`r MC_counter$reset()` `r knitr::knit_child("Exercises/Diff/pine-lead-car.Rmd")`</details>

```{exercise, name="robin-row-boat"}
```
<details>`r MC_counter$reset()` `r knitr::knit_child("Exercises/Diff/robin-row-boat.Rmd")`</details>


```{exercise, name="deer-pitch-saw"}
```
<details>`r MC_counter$reset()` `r knitr::knit_child("Exercises/Diff/deer-pitch-saw.Rmd")`</details>
...
```
<details>`r MC_counter$reset()` `r knitr::knit_child("Exercises/Diff/kitten-put-kayak.Rmd")`</details>


```{exercise, name="rat-take-fork"}
```
<details>`r MC_counter$reset()` `r knitr::knit_child("Exercises/Diff/rat-take-fork.Rmd")`</details>
...
gf_segment(yf + yfend ~ x + xend, data = Segs, 
    ...
    slice_plot(sin(x) ~ x, color="red", domain(x=c(0, pi)),
...
```{r slope-visualization-examples, echo=FALSE, fig.cap="Slope-function visualizations of several naked modeling functions.", fig.height=3, fig.width=7, fig.show="hold", out.width="50%"}
