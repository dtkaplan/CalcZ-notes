...
We've left out the traditional notation for the hump and sigmoid because there isn't a standard one. We will usually denote them as $\dnorm()$ and $\pnorm()$ in order to reinforce the connection to R, but since it's hard to keep straight which one is which, sometimes we'll call the $\hump()$ and $\sigmoid()$.
