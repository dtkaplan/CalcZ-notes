...
gf_point(temp ~ time, data = Stans_data, color="orange") %>%
  ...
  slice_plot(tangent(time) ~ time, color = "sienna", 
             domain(time=c(0,80)))  %>%
   gf_point(y ~ t, data = Pts, color="red", alpha=0.25, size=4, inherit=FALSE) %>%
  gf_line(y ~ t, data = Pts, color="red") %>%
  gf_point(25 ~ t, data = Pts, color="blue", alpha=0.25, size=4) %>%
  ...
  gf_segment(25 + y ~ t+t, data=Pts, color="blue", linetype=2) %>%
  gf_lims(y=c(25, NA)) %>%
  I()
```
...
A set of about a dozen rules will handle all the formulas we'll encounter in this ook or that are commonly used for modeling. In this chapter, we're going to derive only a handful of the rules---those related to the straight-line function. Then we will introduce, without motivation, the rules for the naked and basic modeling functions. In Chapter \@ref(evanescent-h) we'll motivate the rules for the naked modeling functions. In Chapters XXX we'll develop the three rules that correspond to the three forms of creating new functions out of old: linear combinations (the ***sum*** rule), function multiplication (the ***product rule***) and function composition (the ***chain rule***).
...
For a human training to be a symbolic differentiator, it's best simply to memorize the patterns for derivatives of the other naked modeling functions. They are:
...
Note that some of the naked modeling functions have derivatives which are simply naked modeling functions
...
Another couple of naked modeling functions involve division or division by $x$ and multiplication by a scalar. (The second one, $\partial_x \text{hump}(x)$, was not in the original short list)
...
::: {.takenote}
Memorizing the derivatives of the short list of naked modeling functions will make it much easier to follow the rest of the section. Do that now.
:::

Recall that the ***basic modeling functions*** are just the naked functions with clothes on the input. The clothing is can be a simple a scaling function, such as $a\, x$ or $k\, x$  or $\frac{2\pi}{P} x$. As well, the clothing can involve both a scaling and a ***shift***, for instance $a x + b$ or $a (x-x_0)$
Let's give a name to the clothing function and recognize the several varients:

Variations of clothing$(x)$ |    | $\partial_x \text{clothing}(x)$
-----------------------|---------|
$ax$  | scaling | $a$
$kx$  | scaling | $k$
...
Every basic modeling function is a composition of the corresponding naked function with the clothing function:

naked    | clothed   | typically written
---------|-----------|------------------
`exp(x)` | `exp(clothing(x))` | $e^{kx}$
`sin(x)` | `sin(clothing(x))` | $\sin\left(\frac{2\pi}{P} x\right)$
`pnorm(x)` | `pnorm(clothing(x))` | `pnorm(x, mn, sd)`
`dnorm(x)` | `dnorm(clothing(x))` | `dnorm(x, mn, sd)`

To differentiate the basic modeling functions, follow exactly the same rules as for the naked modeling functions, but instead of $x$, write the clothed form of $x$. And then, multiply the whole thing by the derivative with respect to $x$ of the clothing function.

$$\partial_x \left[ f(\text{clothing}(x))\right] = [\partial_x]\left(\strut\text{clothing}(x)\right) \times 
\partial_x \text{clothing}(x)$$
...
```{r Dsin-small-h, echo=FALSE, fig.cap="Graphing the computer's slope function of $\\sin(x)$ for small enough $h$ produces a result inconsistent with the table for the naked modeling functions. Instead of producing a $\\cos(x)$, we get a ragged function."}
...
```
<details>`r MC_counter$reset()` `r knitr::knit_child("Exercises/Diff/finch-trim-kayak.Rmd")`</details>
...
```
<details>`r MC_counter$reset()` `r knitr::knit_child("Exercises/Diff/SIR-dimensions.Rmd")`</details>
