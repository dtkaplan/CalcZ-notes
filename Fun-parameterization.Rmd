...
------------------------|----------------------
Exponential             | $e^{\line(x)}$
Power-law               | $\left[\line(x)\right]^p$
Sinusoid                | $\sin(\line(x))$
Hump                    | $\text{dnorm}(\line(x))$
Sigmoid                 | $\text{pnorm}(\line(x))$
Proportional            | $\line(x)$
Reciprocal              | $1/\line(x)$
...
Remember exercise VIW7T where we looked at where the various naked modeling functions crossed the horizontal or vertical axis. These will often be clues for finding $t_0$ from the graph of a function.
:::
