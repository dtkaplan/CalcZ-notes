...
## Prof. Schaubroeck's *Essential R Commands*
...
* `sin(3)*sqrt(5)`
    - *What it does* Multiplies the sine of 3 times the square-root of 5
    - *Vocabulary*
        - `sin` and `sqrt` are ***functions***
    ...
    - *Notes*
        - Spaces are allowed just about anywhere, but not in the middle of `sin` or `sqrt` (or other function names).
        - If you capitalize one of the functions, you get an error message such as "could not find function \"Sin\"."
        - Later, you will write statements like `sin(x)`. For this to work, `x` must have been previously assigned to a value. If there is no such assignment, you'll get an error code such as "object \"x\" not found." 
