...
width_slider <- sliderInput("p2", "Sigmoid width", min = 0.1, max=5, step=0.1, value=2)
...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("Ansätze: The good, the bad, and the ugly"),
...
server <- function(input, output, server) {
    ...
    points_to_plot <- reactive({
        ...
        Res <- tibble::tibble(
            ...
            x = fun(t),
            x_scaled = input$alpha_val * x,
            dx = dfun(t)
        )
        # are the functions balanced
        success((max(abs(Res$dx - Res$x_scaled)) < 0.01))
        Res
    })
