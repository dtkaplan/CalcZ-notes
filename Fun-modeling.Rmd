...
Seen very abstractly, a mathematical model, as we are using the term, is a set of ***functions*** that represent the relationships between inputs and outputs.  `r mark(900)`
...
`r insert_calcZ_exercise(8.3, "ds4e7",  "Exercises/DD-141Z-04/exponential-bases.Rmd")`
...
b. Find an input value, which we'll call $x_1$ such that $f(x_1)$ is halfway between the baseline $C$ and $f(x_2)$, that is, $$f(x_1) = \frac{f(x_2) - C}{2}$$
c. Then $$k = \frac{\ln(2)}{x_2 - x_1} \approx \frac{0.693}{x_2 - x_1}$$
...
The procedure in Step 3 works for estimating $k$ stemps from a very important property of exponential functions: Exponential functions always double/half at a constant pace. By design, the parameter $k$
...
What’s remarkable about the doubling/halving time is that, for a genuinely exponential function, it doesn’t matter which point we choose for $x_1$. In practice working with graphed data, it's best to choose so that  $f(x_1)$ is discernibly far from the baseline. started with, this property is true across the entire input domain. 
The $2$ in $\ln(2)$ corresponds to idea of doubling/halving. The logarithm converts $2$ to an scale that will generate 2 when exponentiated.
...
`r insert_calcZ_exercise(8.7, "vkwl4", "Exercises/Fun/chicken-choose-vase.Rmd")`

`r insert_calcZ_exercise(8.11, "asevss", "Exercises/Fun/cow-type-kayak.Rmd")`

`r insert_calcZ_exercise(8.15, "j3xe", "Exercises/Fun/maple-hit-saucer.Rmd")`
...
`r insert_calcZ_exercise(8.19, "YELXG", "Exercises/Fun/lamb-talk-gloves.Rmd")`

`r insert_calcZ_exercise(8.23, "VBWD", "Exercises/Fun/spirometer.Rmd")`
...
A gaussian function has two parameters: the location^[That is, the input value at which the function value is largest.] of the peak, which we'll call the ***center*** parameter, and the sideways extent of the gaussian, which we'll call the ***spread***. Figure \@ref(fig:Fun-3C-1) shows a few gaussian functions with different parameters.

```{r Fun-3C-1, echo=FALSE, warning=FALSE, fig.show="hold", fig.height=3, fig.width=6}
slice_plot(dnorm(x, 1, 0.5) ~ x, domain(x=c(-3, 3))) %>%
    gf_labs(title="(a) Gaussian with center=1, spread = 0.5")
slice_plot(dnorm(x, -1, 0.05) ~ x, domain(x=c(-3, 3)), npts=500) %>%
    gf_labs(title="(b) Gaussian with center=-1, spread = 0.05")
slice_plot(dnorm(x, 0.25, 1.2) ~ x, domain(x=c(-3, 3)), npts=500) %>%
    gf_labs(title="(c) Gaussian with center=0.25, spread = 1.2") %>%
...
In R, the name of the sigmoid function is `pnorm()`. The gaussian is `dnorm()`. The parameters that specify center and spread are named `mean` and `sd`. The word "mean" accurately conveys the idea of "center." It would be nice to be able to say that `sd` comes from `s`prea`d`, but in fact `sd` is short for ***standard deviation***, a old-fashioned term familiar to students of statistics.   `r mark(1000)`
...
`r insert_calcZ_exercise(8.25, "IELWV", "Exercises/Fun/hump-intro.Rmd")`



`r insert_calcZ_exercise(8.29, "CKSLE", "Exercises/Fun/sigmoid-intro.Rmd")`

`r insert_calcZ_exercise(8.33, "bllKR", "Exercises/Fun/sigmoid-bath.Rmd")`

`r insert_calcZ_exercise(8.37, "EKCIE", "Exercises/Fun/ebola-sigmoid.Rmd")`

-----

`r insert_calcZ_exercise(8.36, "YLWP1", "Exercises/Fun/flipping-1.Rmd")`

-----


`r insert_calcZ_exercise(8.39, "YLWP2", "Exercises/Fun/flipping-2.Rmd")`
