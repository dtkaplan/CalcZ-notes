...
A good place to start building a model is to pick one of the basic modeling functions. This works surprisingly often. To remind you, here are our nine ***naked*** modeling functions:   `r mark(910)`

```{r child="naked-list.Rmd"}
```
     
The basic modeling functions are the same, but replace the plain $x$ in the naked function with a straight-line function, for instance $ax + b$ or, equivalently $a(x - x_0)$.  In use, the parameter $a$ is often written with some other letter and, often, the $b$ or $-x_0$ part is not needed. 
...
Hump | $\dnorm(x, mn, sd)$ | $mn$ is "mean" or "center"
Sigmoid | $\dnorm(x, mn, sd)$ | $sd$ is "standard deviation" or "spread"
...
Effective modelers treat models with skepticism. They look for ways in which models failure to capture salient features of the real world. They have an eye out for deviations between what their models show and what they believe they know about the system being modeled. They consider ways in which the models might not serve the purpose for which they were developed.  `r mark(1010)`
...
::: {.scaffolding}
In the cooling water example, we're using only a subset of the data collected by Prof. Wagon. The next commands re-create that subset so that you can work with it. They also plot the data and an exponential model.  `r mark(1020)`
```{r results="hide"}
...
Historically, careful analysis of tides led to a highly detailed, highly accurate model: a linear combination of sinusoids with diurnal periods 12.42, 12.00, 12.66, and 11.97 hours as well components with period 23.93, 25.82, 24.07, 26.87, and 24.00 hours. A tide-prediction model is constructed by finding the coefficients of the linear combination; these differ from locale to locale.  `r mark(1060)`


<details>
<summary>`r ex.mark(8.36, "CLESP", fname="inline")` </summary>
Drill on identifying whether the input has been flipped or the output has been flipped or both.
</details>

<details>
<summary>`r ex.mark(8.37, "PSYUV", fname="inline")` </summary>
Drill on finding $x_0$ in the input scaling.
</details>

<details>
<summary>`r ex.mark(8.38, "IWLDN", fname="inline")` </summary>
Introduce the line of identity, when the input equals the output. Which of the basic modeling functions have such a point. (sin, exp, 1/x, x^2, dnorm, pnorm)
</details>

<details>
<summary>`r ex.mark(8.39, "CMWNS", fname="inline")` </summary>
Identify the naked modeling function that has been flipped around the line of identity. Then ask whether the flipped function passes the vertical-line test for the existence of an inverse.
</details>

<details>
<summary>`r ex.mark(8.43, "GSEww", fname="Exercises/Fun/sunset-in-LA.Rmd")` </summary>
