...
$$\underbrace{\int}_{\text{integral sign}} \overbrace{f(x, t)}^{\text{function to be anti-differentiated}} \underbrace{dx}_{\text{identifies 'with respect to' variable}}$$
...
$$\underbrace{\int}_{\text{Sum}} \underbrace{\overbrace{f(x,t)}^\text{height} \overbrace{dx}^\text{width}}_\text{area of box}$$
