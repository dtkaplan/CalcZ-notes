...
- [***Power***] $=  \text{M}\, \text{L}^2/\text{T}^3 $ In calculus, power is the derivative of energy with respect to time.
...
$$\mbox{[Period]}^a \cdot \mbox{[Mass]}^b \cdot \mbox{[Length]}^c \cdot \mbox{[Gravity]}^d = T^a \cdot M^b \cdot L^c \cdot L^d \cdot T^{-2d} = [1]$$
To be dimensionless:
...
$$ \frac{\mbox{Gravity}\cdot \mbox{Period}^2}{\mbox{Length}} \equiv B$$
where $B$ is the size of the dimensionless quantity [1]. Consequently

$$\text{Period} = B \sqrt{\frac{\mbox{Length}}{\mbox{Gravity}} }$$
We can experimentally determine the numerical value of the dimensionless constant $c$ by measuring the period and length of a pendulum and (on Earth) recognizing that gravitational acceleration on Earth's surface is 9.8 meters-per-second-squared. Such experiment and mathematical models using differential equations give $B = (2\pi)^2$.
...
Calculate the dimensionless quantity $B$ in each case. They should all be approximately the same and roughly $B = (2 \pi)^2$.
:::


`r insert_calcZ_exercise("15.4", "mHACgC", "Exercises/Dimension/frog-win-fridge.Rmd")`

`r insert_calcZ_exercise(15.4, "RDTTKW", "Exercises/Dimension/satellite-dimensions.Rmd")`
