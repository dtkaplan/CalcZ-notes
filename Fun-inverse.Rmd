...
We can use this function to solve the problem: How many plows are needed to get the 400-miles of road serviced in 1 day? It's a matter of choosing a suitable method for applying the function to guide us to the answer.  `r note(1210)`
...
An industrious facilities manager might go further. "Today there is 400 miles of road. But next year there will be more, although I don't yet know the exact number. While I'm doing all this math work, I'll write up a memo so that next year, when I know how much road there will be, I can just to some arithmetic." This is a good idea, although a mathematician might prefer the word "function" to "memo [to guide] arithmetic."  `r note(1220)`
...
Straight-line (with non-zero slope), exponential ($e^{kt}$ where $k\neq 0$), and logarithm functions aways have inverse functions. For power-law and sinusoid functions, there can be several (or many) different inverse functions. We'll get to this when we need to.  `r note(1230)`
