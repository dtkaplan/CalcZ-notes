...
```{r heaviside2, echo=FALSE, fig.cap="The Heaviside function is piecewise constat with a discontiuity at $x=0$"}
...
```
Differencing the Heaviside function produces very different functions depending on the value of $h$. The bump near $x=0$ gets taller and taller as $h$ gets smaller. Mathematicians would describe this situation as $$\lim_{h\rightarrow0}{\cal D}_x H(x) \equiv \lim_{h\rightarrow 0} \frac{H(x+h) - H(x)}{h}\ \ \ \text{does not exist}.$$ 
...
Since there is no convergence in the shape of ${\cal D}_x H(x)$ as $h$ gets smaller, it's fair to say that the Heaviside function does not have a derivative. Perhaps a more useful way to think of it is that the "derivative" of the Heaviside function at $x=0$ is unaccountably large as $h\rightarrow 0$.  `r mark(2355)`
...
Another way to think of the existence (or not) of the derivative involves the details of the way the instantaneous rate of change is written. Throughout this book, we've been using one form:
$$\partial_x f(x) \equiv \lim_{h\rightarrow 0} \frac{f(x+h) - f(x)}{h}$$
But there are many other ways we could formula the rate of change. For instance, $$\partial_x f(x) \equiv \lim_{h\rightarrow 0} \frac{f(x) - f(x-h)}{h}$$ is just as good a formulation, as is
$$\partial_x f(x) \equiv \lim_{h\rightarrow 0} \frac{f(x+h) - f(x-h)}{2 h}$$
For any $x$ where a function $f(x)$ has a derivative, each of these three formulations gives the same result.

`r insert_calcZ_exercise("21.1", "yJKLEb", "Exercises/Diff/spider-tug-gloves.Rmd")`

`r insert_calcZ_exercise("21.2", "4Iv7t", "Exercises/Diff/moose-hears-door.Rmd")`


`r insert_calcZ_exercise("21.3", "UNeewh", "Exercises/Diff/maple-sail-boat.Rmd")`
