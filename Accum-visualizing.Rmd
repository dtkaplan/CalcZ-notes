...
Now imagine that we sliced up $F(x)$ over small sub-domains of $x$, as in Figure \@ref(fig:F-slice1). That is, we approximated $F()$ piecewise locally.

```{r F-slice1, echo=FALSE}
...
Start by reading off the function value from the last point in the left-most segment. That's been marked in Figure \@ref(fig:F-slice1) with a blue dot. The function value at that dot is 7.072. 
...
One <- Pts %>% mutate(add = 7.072*(segnum==1), 
                      ymin=ymin + add,
                      ymax=ymax + add)
gf_segment(ymin + ymax ~ xmin + xmax, data = One) %>%
  gf_point(7.072 ~ -2.995, color="blue", size=2) %>%
...
Two <- One %>% mutate(add = 4.198*(segnum==2), 
                      ymin=ymin + add,
                      ymax=ymax + add)
gf_segment(ymin + ymax ~ xmin + xmax, data = Two) %>%
  gf_point(7.072 ~ -2.995, color="blue", size=2) %>%
  gf_point(4.198 ~ -.9919, color="blue", size=2) %>%
...
```{r F-slice-many, echo=FALSE, fig.cap="Using smaller segments (width $h=0.1$) gives each segment a simple shape."}
