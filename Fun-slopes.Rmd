...
Each of the words in this definition is used in everyday speech. Still they require some explanation because we are using them to describe mathematical topics. The task of this chapter is to connect, in a preliminary way, the words "function,", "model," "continuous," and "change" to their meanings in calculus.
...
As you'll see, when we need a new function for some purpose, we practically always build it out of existing functions. That brings us back to the ***naked modeling functions***. The collection listed above is not only familiar to many students, but are far and away the most commonly used in building models.

And yet ... We are being a bit disrespectful in calling them ***naked modeling functions***. The word "naked" suggests that they are not yet ready to go out into the real world. Later, we will translate the naked modeling functions into ***basic modeling functions*** that are dressed and ready to engage the real world. 

## Change and slope

You already know what "change" means. In mathematics, and especially the mathematics of functions, change has a very simple meaning that you have already touched on in your previous math education.
...
Review how to compute the slope.
