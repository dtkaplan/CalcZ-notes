...
This situation arises particularly when $f(x)$ and $g(x)$ are pattern-book functions for which you already have memorized $\partial_x f(x)$ and $\partial_x g(x)$ or are basic modeling functions whose derivatives you will memorize in Chapter \@ref(prod-comp-rules).
...
As with all derivatives, the product rule is based on the instantaneous rate of change $$\partial_x h(x) \equiv \lim_{h\rightarrow 0} \frac{h(x+h) - h(x)}{h}$$

We also need two other statements about $h$ and functions:

1. The derivative is the slope of the function, so $f(x+h) = f(x) + h f'(x)$.
...
$$\partial_x \left[\strut f(x) g(x) \right] \equiv \frac{f(x+h) g(x+h) - f(x) g(x)}{h} = \\
\,\\
...
\,\\
= f'(x) g(x) + f(x) g'(x) + \cancelto{0}{h f'(x) g'(x)} = \\
\\\,\\
=f'(x) g(x) + f(x) g'(x)
$$
...
Again, in practice, models have memorized the chain rule; they don't derive it each time they use it!
