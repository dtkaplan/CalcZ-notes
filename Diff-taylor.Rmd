...
2. Creating a function to capture the patterns in data, as in Chapter \@ref(local-approximations). It turns out that this is a dubious practice. We discuss the reasons why so that you can know to avoid using high-order polynomials to fit data.
...
the output is always zero when $x=x_0$, which will be a matter of considerable importance as we go on. Also, note that we're using square braces $\left[\ \ \right]$ simply to make it completely unambiguous what is being exponentiated.
...
This format is convenient because in finding the $a_0$, $a_1$, $\ldots$ for approximating a function $f(x)$ in the neighborhood of $x_0$, we have a way to calculate quickly the value of $a_0$.  Note that at $x=x_0$, all the terms in the polynomial go to zero except the first, so we know $a_0 = f(x_0)$.
...
```
<details>`r MC_counter$reset()` `r knitr::knit_child("Exercises/Diff/approx-orange.Rmd")`</details>

-----

```{exercise, name="approx-blue"}
```
<details>`r MC_counter$reset()` `r knitr::knit_child("Exercises/Diff/approx-blue.Rmd")`</details>
