...
## Basics of polynomials with one input {#polynomial-basics}
...
The ***domain*** of polynomials, like the power-law functions they are assembled from, is the entire number line: $-\infty < x < \infty$. But for the purposes of understanding the shape of high-order polynomials, it's helpful to divide the domain into three parts: a ***wriggly domain*** at the center and two ***tail domains*** to the right and left of the center.
...
```
Figure \@ref(fig:wriggly-polynomial) shows a 7-th order polynomial---that is, the highest-order term is $x^7$. In one of the tail domains the function value heads off to $\infty$, in the other to $-\infty$. This is a necessary feature of all odd-order polynomials: 1, 3, 5, 7, ...
...
In the wriggly domain in Figure \@ref(fig:wriggly-polynomial), there are six argmins or argmaxes. 
...
P %>%
  gf_refine(scale_y_log10(breaks=10^(seq(-16, 0, by=2))), scale_x_log10(breaks=breaksx, labels=as.character(breaksx)))
...
0   | $\sin(x) \left.{\Large\strut}\right|_{x=0} = 0$ | $\left( 1 - \frac{1}{6}x^3\right)\left.{\Large\strut}\right|_{x=0} = 0$ 
  ...
  2   | $-\sin(x) \left.{\Large\strut}\right|_{x=0} = 0$ | $\left(- \frac{6}{6} x\right) \left.{\Large\strut}\right|_{x=0} = -1$
...
Let's construct a 3th-order Taylor polynomial approximation to $f(x) = e^x$ around $x=0$.

We know it will be a 3th order polynomial:
...
Result: the 3rd-order Taylor polynomial approximation to the exponential at $x=0$ is $$g_{\exp}(x) = 1 + x + \frac{1}{2} x + \frac{1}{2\cdot 3} x^2 + \frac{1}{2\cdot 3} x^3 +\frac{1}{2\cdot 3\cdot 4} x^4$$

Figure \@ref(fig:taylor-exp-3) shows the exponential function $e^x$ and its 3th-order Taylor polynomial approximation near $x=0$:

```{r taylor-exp-3, echo=FALSE, class.source="code-hide", fig.cap="The 3th-order Taylor polynomial approximation to $e^x$ arount $x=0$"}
...
```{r taylor-exp-5, echo=FALSE, class.source="code-hide", fig.cap="The error from a 3th-order Taylor polynomial approximation to $e^x$ around $x=0$ is a power-law function with exponent $4$.", out.width="50%", fig.show="hold"}
...
```{r sinc-near-zero, fig.cap="To judge from this plot, sin(0) = 1."}
slice_plot(sin(x) / x ~ x, domain(x=c(-10,10)))
```
...
Both $\sin(x)/x$ and $\sin(x) / x^3$, evaluated at $x=0$ involve a divide by zero. Both are the indeterminant form 0/0 at $x=0$. But the graph of $\sin(x) / x^3$ (see Figure \@ref(fig:sinc2)) is not we'll behaved.  $\sin(x) / x^3$ does not have any particular value at $x=0$; instead, it has an asymptote. 

```{r sinc2, fig.cap="Left: The graph of $\\sin(x)/x$ versus $x$, closer to zero using semi-log axes. Right: The graph of $\\sin(x)/x^2$. The vertical scales on the two graphs are utterly different."}
slice_plot(sin(x) / x ~ x, domain(x=c(-0.1, 0.1))) %>%
...
Evan at $x=0$, there's nothing indeterminant about $1 + x^2/6$; it's simply 1.
...
$$\lim_{x\rightarrow 0} \frac{\sin(x)}{x^3} = \frac{\lim_{x\rightarrow 0} \cos(x)}{\lim_{x \rightarrow 0} 3x^2} = \frac{1}{0} \ \ \text{indeterminant}!$$
...
`r insert_calcZ_exercise("26.02", "3EdMBL", "Exercises/Diff/birch-lie-sheet.Rmd")`

`r insert_calcZ_exercise("26.04", "o6dP4k", "Exercises/Diff/pig-iron-closet.Rmd")`

`r insert_calcZ_exercise("26.06", "Co1Ekt", "Exercises/Diff/dog-feel-boat.Rmd")`

`r insert_calcZ_exercise("26.08", "llX7EF", "Exercises/Diff/calf-tear-fridge.Rmd")`

`r insert_calcZ_exercise("26.10", "ecdVKx", "Exercises/Diff/frog-throw-screen.Rmd")`


`r insert_calcZ_exercise("26.12", "682lsB", "Exercises/Diff/local-shift.Rmd")`

`r insert_calcZ_exercise("26.14", "cQT2v6", "Exercises/Diff/dolphin-mean-linen.Rmd")`

`r insert_calcZ_exercise("26.16", "jAOmME", "Exercises/Diff/birch-dig-radio.Rmd")`
