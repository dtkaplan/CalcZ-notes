...
slice_plot(dlen(66.5*pi/180, d) ~ d, domain(d=c(0,365)), npts=5000) %>%
  gf_hline(yintercept=c(0, 24), color="dodgerblue") %>%
  gf_lims(y=c(0,24))
```
...
Linear combination and composition of functions are often used hand-in-hand to construct models. For instance, our model of the data from cooling water was a linear combination of a constant function and a sinusoidal function $$\text{temperature}(t) \equiv 25.9 + 60.7\, e^{-0.019 t}$$
Similarly, our model from the Providence tide data was
$$\text{tide}(t) \equiv 1.02 + 0.50 \sin(2\pi (t - 15.4)/12.56)$$
...
::: {.objectives}
```{r echo=FALSE, results="asis"}
state_objective("Fun-1C-a", "Review (from previous math classes) the parameters of the straight-line function: y-intercept and slope.")
state_objective("Fun-1C-b", "Use a straight-line function in a modeling context.")
state_objective("Fun-1C-c", "Spot discrepancies of data from a straight-line model.")
```
:::
...
slice_plot(exp(1.5*x) ~ x, domain(x=c(-2, 2)), color="dodgerblue", label_text="k=1.5") %>%
  slice_plot(exp(0.5*x) ~ x, domain(x=c(-2, 2)), color="green", label_text="k=0.5") %>%
  slice_plot(exp(-1.0*x) ~ x, domain(x=c(-2, 2)), color="orange3", label_text="k=-1.0") %>%
...
::: {.objectives}
```{r echo=FALSE, results="asis"}
state_objective("Fun-2b", "Know that exponential functions have the property that their slope (rate of change) at each input is proportional to the height of the function at that same input. And that the slopes approach zero in one of the directions.")
```
:::
...
It's worth unpacking that simple statement a bit. We have an exponential function $g(x) \equiv e^x$. The slope of the exponential function is, as we have described, $$\text{slope of}\ g(x)\ \text{is} \ \frac{e^{x + h} - e^x}{h} = \frac{e^x e^h - e^x
}{h} = e^x \frac{e^h - 1}{h}$$
Look carefully at the term $(e^h - 1)/h$. Notice that there is no $x$ in it. Thus, the function $(e^h - 1)/h$ is *constant with respect to $x$*. In other words $$\text{slope of}\ g(x)\ \text{is} \ \text{constant} \times e^x$$.
...
`r XXXinsert_calcZ_exercise("XX.XX", "9sles", "Exercises/Fun/chicken-choose-vase.Rmd")`

`r XXXinsert_calcZ_exercise("XX.XX", "3sdwB", "Exercises/Fun/cow-type-kayak-new.Rmd")`
...
::: {.objectives}
```{r echo=FALSE, results="asis"}
state_objective("Fun-2c", "Understand that $e^{kt}$, $2^{k_2 t}$, and $10^{k_{10} t}$ are all the same function, when $k = k_2 \\ln(2) = k_{10} \\ln(10)$. Relate $k_2$ to the doubling time and $k_{10}$ to the time to increase 10-fold.")
```

Revised from "Know how changing the values of C and k affect the properties of the exponential model $g(x) \equiv C e^{kt}$ (table page 62)"
:::
...
::: {.objectives}
```{r echo=FALSE, results="asis"}
state_objective("Fun-2d", "Distinguish between exponential functions and power-law functions.")
state_objective("Fun-2e", "Relate power-law functions to physical quantities.")
```
:::
...
slice_plot(t^3 ~ t, domain(t=c(-2, 2)), color="dodgerblue", 
           label_text = "t^3", label_x = 0.1) %>%
  slice_plot(t^ 1.5 ~ t, color="orange", label_text="t^{1.5}") %>%
  slice_plot(t^-.5 ~ t, color="black", label_text="sqrt(t)", 
             label_x = 0.51) %>%
  slice_plot(t^-2 ~ t, color="orange3", label_text="t^{-2}", label_x=0.05, domain(t=c(.2,2))) %>%
slice_plot(t^-2 ~ t, color="orange3", domain(t=-c(.2,2))) %>%
...
::: {.objectives}
```{r echo=FALSE, results="asis"}
state_objective("Fun-3a1", "Understand a logarithm (base-10) as signifying the magnitude in terms of the number of digits.") 
state_objective("Fun-magnitude", "See $\\log_2()$ and $\\log_{10}()$ as counting digits in base-2 and -10 respectively.")
```
:::
...
::: {.objectives}
```{r echo=FALSE, results="asis"}
state_objective("Fun-3B-a", "Become familiar with the sinusoid functions and the period parameter. Be able to identify the period from the graph of a sinusoid-like function.")
```
:::
...
slice_plot(sin(2*pi*t/0.3) ~ t, domain(t=c(-2,2)), color="dodgerblue", npts=500) %>%
...
You can choose any starting value you like to define a cycle. For the purposes of measuring the period from a graph, I find it convenient to start at the peak, that is, somewhere that $\text{sinusoid}(t) = 1$ or the trough (that is, $\text{sinusoid}(t) = 1$). No matter what you choose, you will get the same answer.
...
The domain of $\text{sinusoid}(t)$ is the entire number line. The range is the interval from -1 to 1, and the ink in a sinusoid graph is balanced on either side of zero.
...
::: {.objectives}
```{r echo=FALSE, results="asis"}
state_objective("Fun-3B-a", "Recognize general graphical features of sigmoidal functions and their associated hump functions")
state_objective("Fun-3B-b", "Explain why the slope function of a sigmoidal function is a hump function.")
state_objective("Fun-3B-c", "OPTIONAL Be aware of different formulations for the hump sigmoidal functions.")
```
:::
...
slice_plot(dnorm(x, 0.25, 1.2) ~ x, domain(x=c(-3, 3)), npts=500) %>%
  ...
  gf_hline(yintercept = dnorm(1.2+0.25, 0.25, 1.2), color="red", size=0.5) %>%
    gf_segment(0.2016 + 0.2016 ~ -0.95 + 1.45, color="red", size=2, alpha=.03)
```
...
slice_plot(logistic_hump(t, k=1.5/1, t_0=0.5) ~ t, domain(t=c(-3,3)),
  ...
  slice_plot(dnorm(t, 0.5, 1) ~ t, color="dodgerblue")
slice_plot(logistic(t, k=1.5/1, t_0=0.5) ~ t, domain(t=c(-3,3)),
  ...
  slice_plot(pnorm(t, 0.5, 1) ~ t, color="dodgerblue")
```

The sigmoidal ***logistic function*** (drawn in red in Figure \@ref(fig:Fun-3C-3)) is a favorite among mathematicians. Its formula is $$\text{sigmoid}_\text{logistic}(x) \equiv  \frac{1}{1 + e^{-k(t-t_0)}}$$ The parameter $t_0$ is the ***center***. The ***spread*** (as we defined it earlier along the lines of half-width at half-height) is roughly $1.5/k$.

Since the hump function is the slope of the sigmoid function, the formula for $\text{sigmoid}_\text{logistic}(x)$ gives directly the hump's formula:
$$\text{hump}_\text{logistic}(x) \equiv \frac{k e^{-k(t-t_0)}}{(1 + e^{-k(t-t_0)})^2}$$
The ***Gaussian function*** (drawn in blue) is much more widely used in science and statistics.  It's formula is
$$\text{hump}_\text{gaussian}(x) \equiv {\small \frac{1}{\sqrt{2\pi \sigma^2}}} e^{-\frac{(t - t_0)^2}{2 \sigma^2}}$$
...
An amazing bit of trivial is that there is no formula for $\text{sigmoid}_\text{gaussian}()$.

You might ask, "If there is no formula, how can we use the function?" The answer is that even though there is no formula, there is an algorithm for calculating the value of $\text{sigmoid}_\text{gaussian}(t)$. We're now starting to get into the center of the realm of calculus, which provides us new concepts and notation for describing the correspondance between functions. We'll need much of CalcZ to introduce and master these concepts and their notation. For now, we will let the notation make a cameo appearance, a premonition of what is to come.  

To say, "the slope of the sigmoid function is the hump function," we  write $$\partial_t \text{sigmoid}(t) = \text{hump}(t)$$

To describe an *algorithm* for computing the output of the sigmoid function, we need somehow to move the $\partial_t$ from the left side of the equation to the right side. This is a subtle operation and is conventionally written $$\text{sigmoid}(t) = \int_{-\infty}^t \text{hump}(x) dx$$ 
...
One helpful way to get to know the pattern-book functions individually is to graph them. People have a natural talent to remember shapes and distinguish one from another; your job is to associate each shape with it's name.
...
* If $p$ is an integer, then the domain is the entire number line, $-\infty < x < \infty$
...
slice_plot(x^2.5 ~ x, domain(x=c(0,3))) %>%
  ...
  gf_text(5 ~ -2, label="When p = 2, 4, 6, 8, ...", color="dodgerblue") 
  
slice_plot(abs(x)^2.5 ~ x, domain(x=c(0,3))) %>%
  slice_plot(-abs(x)^2.5 ~ x, domain(x=c(-3, 0)), size=2,
             color="dodgerblue", alpha =0.2) %>%
  gf_labs(title = "Power law with p > 0") %>%
  gf_text(5 ~ -2, label="When p = 1, 3, 5, 7, ...", color="dodgerblue") 

slice_plot(x^-1 ~ x, domain(x=c(0.1,3))) %>%
  slice_plot(1/abs(x) ~ x, domain(x=c(-3, -0.1)), size=2, alpha=0.2, color="dodgerblue") %>%
  gf_text(5 ~ -2, label="When p = 2, 4, 6, 8, ...", color="dodgerblue") %>%
  gf_labs(title = "Power law with p < 0")

slice_plot(x^-1 ~ x, domain(x=c(0.1,3))) %>%
  slice_plot(-1/abs(x) ~ x, domain(x=c(-3, -0.1)), size=2, alpha=0.2, color="dodgerblue") %>%
  gf_text(-4.5 ~ -2, label="When p = -1, -3, -5, -7, ...", color="dodgerblue", alpha = 0.3) %>%
  gf_labs(title = "Power law with p < 0")
```

::: {.notethis}
With some practice, you should be able to sketch roughly any of the pattern-book functions from memory. When you can do this, you'll have an easier time envisioning which function might best suit a situation.

In addition to the shape, here are some facts about the pattern-book functions that will let you sketch accurately.
