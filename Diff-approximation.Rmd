...
It's often the case that we don't know a definite functional form for a relationship. Instead we know about the relationship only through data. It's helpful to be able to create a function that depicts the relationship implicit in the data. Local approximations provide a ready method for creating functions that can represent a wide range of relationship patterns.
...
$$\text{body_fat}(h, w) \equiv c_0 + c_h h + c_w w + c_{hw} h w$$
...
`r insert_calcZ_exercise("24.02", "ckslw", "Exercises/Diff/rooster-pink.Rmd")`

`r insert_calcZ_exercise("24.04", "ikdlx", "Exercises/Diff/rooster-violet.Rmd")`
