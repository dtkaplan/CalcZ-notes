...
Let's start with a plain statement of the rule, then show why it's right. Written in terms of the pronoun functions $f()$ and $g()$, the ***chain rule*** for the derivative of the composition $f(g(x))$ is
...
::: {.example data-latex=""}
Let's use the chain rule to find the derivative $\partial_x e^{2x}$.
...
::: {.example data-latex=""}
The chain rule is used in a clever way to find a formula for $\partial_x \ln(x)$. 
...
`r insert_calcZ_exercise("22.12", "0yrIXe", "Exercises/Diff/monkey-forget-tv.Rmd")`
