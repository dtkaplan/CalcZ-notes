...
Division by zero produces `Inf`, whose name is reminiscent of "infinity." `Inf` infiltrates any calculation in which it takes part:  `r note(1510)` 
```{r warning=FALSE}
3 * Inf
sqrt(Inf)
0 * Inf
...
The table shows computer notation for some common sorts of questions.  `r note(1520)` 
...
::: {.intheworld}
Figure \@ref(fig:gas-use-2) is a graph of monthly natural gas use in the author's household versus average temperature during the month. (Natural gas is measured in cubic feet, appreviated *ccf*.)  `r note(1530)` 
