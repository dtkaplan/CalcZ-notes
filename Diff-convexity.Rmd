# Concavity, curvature, and smoothness
...
## Quantifing concavity, curvature, and smoothness

It often happens in building models that the modeler (you!) knows something about the concavity, curvature, or smoothness of a function. For example, ***concavity*** is important in classical economics; the curve for supply is concave down while the curve for demand is concave up. For a train, car, or plane, there are forces that depend on the curvature of the track, road, or trajectory. If you are designing a road, you'll need to calculate the curvature in order to know if the road is safe at the indicated speed. And if you are programming a robot arm, you need to measure the smoothness of of motion in order to avoid putting excessive force on the arm itself or the object it is holding.

It turns out that quantifying the properties of functions or shapes is naturally done by calculating derivatives.  `r mark(2240)`
...
slice_plot(f(x) ~ x, domain(x=c(0, 4)))
```
...
$$\text{concavity_of_f}(x) \equiv\ \text{rate of change in}\ \partial_x\, f(x) = \partial_x [\partial_x f(x)] \\
\\
= \frac{\partial_x f(x+h) - \partial_x f(x)}{h}$$
...
Such a double derivative of a function $f(x)$ is called the ***second derivative*** of $f(x)$. The second derivative is so important in applications that it has it's own compact notation:
...
dxx_f <- D(f(x) ~ x & x)
```
Notice that we're doubling up on the `x` in the right-hand side of the tilde expression.

`r insert_calcZ_exercise("XX.XX", "iIXOYZ", "Exercises/Diff/crow-write-chair.Rmd")`
...
```{r echo=FALSE}
curve_circle <- function(ftilde, x0=0) {
  ff <- makeFun(ftilde)
  df <- D(ff(x) ~ x)
  ddf <- D(ff(x) ~ x & x)
  ...
  centerx <- x0 + cos(angle)/K
  centery <- ff(x0) + sin(angle)/K
...
```{r}
slice_plot(x^2 ~ x, domain(x=c(-2,2))) %>%
  gf_path(y ~ x, data = curve_circle(x^2 ~ x, 1), color="tan", size=2, alpha=0.4) %>%
  gf_path(y ~ x, data = curve_circle(x^2 ~ x, 0.0), color="blue", size=2, alpha=0.5) %>%
  gf_path(y ~ x, data = curve_circle(x^2 ~ x, -1.5), color="green", size=2, alpha=0.5) %>%
  gf_point(1 ~ 1) %>%
  gf_point(0 ~ 0) %>%
  gf_point(2.25 ~ -1.5) %>%
...
AN EXERCISE ON CALCULATING THE CURVATURE OF THE sin at the zero-crossings and at the argmax and argmin.
```{r}
f <- makeFun(sin(x) ~ x)
slice_plot(f(x) ~ x, domain(x=c(-3,3))) %>%
  gf_path(y ~ x, data = curve_circle(f(x) ~ x, -pi/2), color="tan", size=2, alpha=0.4) %>%
  gf_path(y ~ x, data = curve_circle(f(x) ~ x, 1), color="blue", size=2, alpha=0.5) %>%
  gf_path(y ~ x, data = curve_circle(f(x) ~ x, +pi/2), color="green", size=2, alpha=0.5) %>%
  gf_point(1 ~ pi/2) %>%
  gf_point( sin(1)~ 1) %>%
  gf_point(-1 ~ -pi/2) %>%
