...
::: {.intheworld}
To illustrate the cultural admiration for abstraction in mathematics, consider this famous poem by a University of Oxford mathematician, Charles Lutwidge Dodgson (1832-1898):
...
```{exercise, name="convention-drill"}
```
<details>`r MC_counter$reset()` `r knitr::knit_child("Exercises/Fun/convention-drill.Rmd")`</details>
...
```{exercise, fnotation}
```
<details>`r MC_counter$reset()` `r` knitr::knit_child(exercise_file("02", "function-notation.Rmd"))`</details>
