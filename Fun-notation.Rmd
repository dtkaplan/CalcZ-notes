...
0 = (x+3)(x-2)
```
Once you've made your choice, open a `r sandbox_link()` to see if you got it right.

Try one line at a time, and erase it before trying another line.  If the code is invalid, pressing "Run code" will lead to an error message below the sandbox. The valid line will not lead to any error message.
:::
...
Most of the time in this course, the functions you define will be described by either 

i. A ***formula***, such as `3*x + 2` or $x^2 \sin(y)$ 
ii. Another kind of computation of the sort not typically covered in high-school math: an iterative process, a branching process, a neural network, and so on. These processes are usually expressed using a computer programming language and so they are unfamiliar to many students.
