...
`r insert_calcZ_exercise("19.1", "kelwx", "Exercises/Diff/tilde-function.Rmd")`
...
```{r check-dg, fig.cap="A check that zero-crossings (blue) of the derivative function (tan) correspond to inputs where the original function is flat (black)." }
...
::: {.takenote  data-latex=""}
In modeling work, there's nothing wrong with an approximation so long as it is good enough for your purposes. We picked the value 0.1 for our definition of the slope function because it works very well with the pattern-book functions. Here, "very well" means you can't easily see in the graph any deviation compared to the exact derivative. `r mark(2190)`
...
`r insert_calcZ_exercise("19.2", "K05LrF", "Exercises/Diff/fly-speak-canoe.Rmd")`

`r insert_calcZ_exercise("19.3", "n78LrF", "Exercises/Diff/wolf-talk-kayak.Rmd")`
...
```{r}
#two arguments, one with a default value
...
In practice, instead of home-brewed functions like `slopeFun()`, you can use the R/mosaic `D()` instead, which gives the "exact" results whenever it can.
...
`r insert_calcZ_exercise("19.5", "qCfet3", "Exercises/Diff/doe-pay-kitchen.Rmd")`

`r insert_calcZ_exercise("19.6", "ykels", "Exercises/Diff/finch-trim-kayak.Rmd")`
