...
A function is ***continuous*** if you can trace out the graph of the function without lifting your finger from the page. All of the pattern-book functions are continuous except for the reciprocal function and the power law functions with negative exponents (of which the reciprocal is one example: $1/x = x^{-1}$).

Consider the power-law function graphed in Figure \@ref(fig:for-words). This is continuous almost everywhere. There's just one input where the continuity fails, $x=0$. For inputs $x < 0$, the function is negative. For inputs $0 < x$, the function is positive. A continuous function whose value switches from positive to negative must, at some input value, be zero^[This is actually formalized in the intermediate value theorem which says that any function $f()$ that is continuous on an interval from $a$ to $b$ must necessarily output every value between $f(a)$ and $f(b)$ at some input within the interval.]. But the function $1/x$ is not zero at $x=0$; it is undefined. In other words, $1/x$ is discontinuous at $x=0$. `r mark(433)`
