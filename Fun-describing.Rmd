...
The next short sections describe five simple shape concepts: concavity, continuity, periodicity, asymptotes, and local extrema.
...
The slope of a function at a given input tells how fast the function output is increasing or decreasing as the input changes slightly. Slope is about how output change.

***Concavity*** is about how slope changes. For instance, a function might be growing slowly in some region of the domain and then gradually shift to larger growth in an adjacent region. Or, a function might be decaying steeply and then gradually shift to a slower decay. Both of these are instances of ***positive concavity***. The opposite pattern of change in slope is called ***negative concavity***. If the slope doesn't change at all---only straight-line functions are this way--- the concavity is zero.

Concavity has a very clear appearance in a function graph. If a function is positive concave in a region, the graph looks like a smile. Negative concavity looks like a frown. Zero concavity is a straight line.
...
The idea of representing with sinusoids phenomena that are almost but not exactly periodic, for instance a communications signal or a vibration, is fundamental to to many areas of physics and engineering.
...
2. When the **output** of a function gets bigger and bigger in size, going to $\infty$ or $-\infty$ without the input doing likewise. The visual appearance on a graph is like a sky-rocket: the output changes tremendously fast even though the input changes only a little.  This is called a ***vertical asymptote***. The power-law function $x^{-1}$ has a ***vertical asymptote*** of $\infty$ as $x \rightarrow 0$ from the right, and another vertical asymptote of $-\infty$ as $x \rightarrow 0$ from the left. 
