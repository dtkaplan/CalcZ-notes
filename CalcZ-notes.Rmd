...
Not yet, but at the appropriate point in Block 1 you should be able to list all 7 basic modeling functions and say which of these features are relevant to each.
...
Newton considered the problem for continuous time rather than Galileo's discrete time. He reframed the slope function from the big increments of the slope operator ${\cal D}_t$ to imagined vanishingly small increments of a operator that we shall cal $\partial_t$ and which is now called ***differentiation***.
...
slice_plot(Dexp(t, h=1) ~ t, domain(t=c(-2, 2)), label_text="h=1", label_x=.4, color= rain[2]) %>%
  ...
  gf_lims(y=c(0,8))
```
These numerical experiments with the logarithm and exponential functions confirm that, as $h$ gets small, the slope function doesn't depend on $x$. And, we find that:
