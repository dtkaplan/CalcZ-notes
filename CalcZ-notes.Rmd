...
2. Basics of modeling functions
    a. NTI
        ...
        i. Show students graph of 5 naked functions, have them pick which is which
        i. Points out that a *truly* naked straight-line is just $x$, and use that as the introduction to parameterization
        i. Show scatter plot of $f(x)\equiv -x+2$ and explain why $f(x)\equiv x$ is not a good model to motivate basic modeling function
    a. Lecture Topics
        i. Naked modeling functions
        i. Basic Modeling functions and their behaviors based on their parameters
    a. Outcomes
        i. Know graphs, domains, ranges, horizonal/veritcal intercepts & asymptotes (if any) for all the naked modeling functions
        i. Recognize that functions are a way of modeling relationship in the real-world
        i. Given parameters, describe how those parameters affect the shape of the function
    a. Readings: Section \@ref(naked-intro) and Chapter \@ref(structure-of-a-function)
    a. Additional Resources
...
```
<details>`r knitr::knit_child("Exercises/Fun/snail-tear-blanket.Rmd")`</details>


```{exercise, name="goat-dive-saucer"}
```
<details>`r knitr::knit_child("Exercises/Fun/goat-dive-saucer.Rmd")`</details>

```{exercise, name="chicken-show-map"}
```
<details>`r knitr::knit_child("Exercises/Fun/chicken-show-map.Rmd")`</details>


```{exercise, name="eagle-bring-car"}
```
<details>`r knitr::knit_child("Exercises/Fun/eagle-bring-car.Rmd")`</details>

```{exercise, name="pig-look-rug"}
```
<details>`r knitr::knit_child("Exercises/Fun/pig-look-rug.Rmd")`</details>
...
gf_point(temp ~ time, data = Stans_data, color="orange") %>%
  ...
  slice_plot(tangent(time) ~ time, color = "sienna", 
             domain(time=c(0,80)))  %>%
   gf_point(y ~ x, data = Pts, color="red", alpha=0.25, size=4, inherit=FALSE) %>%
  gf_line(y ~ x, data = Pts, color="red") %>%
...
<!--chapter:end:Diff-cont-and-smooth.Rmd-->
