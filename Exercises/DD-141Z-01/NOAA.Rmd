...
knitr::include_graphics(exercise_file("01", "www/heat-index.png"))
```


```{r daily-digital-01-QA2, echo=FALSE}
askMC(
  prompt = "A) What are the inputs to the heat-index function",
...
askMC(
  prompt = "B) For inputs of 70% relative humidity and $88^{\\circ}$ F, what are the outputs of the three functions?",
...
askMC(
  prompt = "C) Holding the relative humidity at 70%, how much would the ambient temperature have to increase (from $88^\\circ$ F) to change the caution-level output to \"dangerous\"?",
...
askMC(
  prompt = "D) From a starting point of $88^\\circ$ F and 70% humidity, what is the slope of the increase in heat index when moving to 80% humidity.",
...
askMC(
  prompt = "E) What is the heat-index output when the inputs are 52% relative humidity and $91^\\circ$ F? Choose the best answer.",
...
askMC(
  prompt = "True or false: The caution-level output could have been presented as a function of just one variable, rather than needing both temperature and humidity both.",
...
knitr::include_graphics(exercise_file("01", "www/weather-service-heatindex.png"))
```

[Source link](https://www.weather.gov/images/oun/wxsafety/summerwx/heatindex.gif)
