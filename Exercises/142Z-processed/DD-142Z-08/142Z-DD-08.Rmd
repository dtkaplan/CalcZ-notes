...
etude2::set_master_id("DD-08") # Add the DD number
etude2::etude_document_name("DD-142Z-08") # Add the DD number
...
a. Understand the relationship between the hump function and the sigmoidal function. 
...
iv. logarithm
v. hump functions, a go-to function for something that happens locally
...
> The derivative of a $\left\{\begin{array}{c}\text{exponential}\\\text{power-law}\\\text{sinusoid}\end{array}\right\}$ is a $\left\{\begin{array}{c}\text{exponential}\\\text{power-law}\\\text{sinusoid}\end{array}\right\}$

The same property can also be stated in terms of anti-derivatives :

> The anti-derivative of a $\left\{\begin{array}{c}\text{exponential}\\\text{power-law}\\\text{sinusoid}\end{array}\right\}$ is a $\left\{\begin{array}{c}\text{exponential}\\\text{power-law}\\\text{sinusoid}\end{array}\right\}$ $+ C$.
...
derivative of a $\left\{\begin{array}{c}\text{sigmoid}\\\text{hump}\end{array}\right\}$ is a $\left\{\begin{array}{c}\text{hump}\\\text{sigmoid}\end{array}\right\}$
...
IN DRAFT ... Present the gaussian. Show the analytic form for the gaussian. Expectation value is the "center" parameter. The density function and the cumulative distribution function are so important that they have been given names in R, `dnorm()` and `pnorm()` respectively.

Here's another probability density function that looks very complicated but is nonetheless important in many fields. Physicists and engineers tend to call it the "Gaussian" distribution, but in many other fields it is simply called the "normal" distribution, that's how common it is. 
