...
gf_point(.4945 ~ .4945, color="red", alpha=0.3, size=3) %>% 
  gf_point(0.5340 ~ 0.5340, color="green", alpha=0.3, size=3) %>%
  gf_line(y ~ x, data = Pts) %>%
  gf_abline(intercept=0, slope=1, color="blue", alpha=0.5) %>%
  gf_refine(coord_fixed(ylim=c(0,1))) %>%
  gf_labs(y = expression(xi[n+1]), x = expression(xi[n])) 
```

The symbol in the axes labels is the Greek letter $\xi$, pronounced "ex-eee" and the equivalent of the Roman letter "x". We're using Greek here to point out that it doesn't really matter what you call the variable being used in an expression like $\xi_{n+1} = g(\xi_n)$. The scale of the variable $\xi$ runs from zero to one. But sometimes it's more convenient to create a new variable, related to the original one, that puts zero right in the middle.
...
P <- gf_point(.4945 ~ .4945, color="red", alpha=0.3, size=3) %>% 
  gf_line(y ~ x, data = Pts) %>%
  gf_abline(intercept=0, slope=1, color="blue", alpha=0.5) %>%
...
gf_refine(
  ...
  gf_vline(xintercept=0.4945, color="red", alpha=0.2) %>%
  gf_hline(yintercept=0.4945, color="red", alpha=0.2)
```

In terms of $x$, the dynamics and corresponding solution are approximately $$\underbrace{x_{n+1} = -0.3960\, x_n}_\mbox{dynamics}\ \ \ \ \implies\ \ \ \ \underbrace{x_n = x_0 (-0.3960)^n}_\mbox{solution}$$
...
askMC(
  "What is the location of the fixed point? (We'll denote this by $\\xi^\\star$.)",
  "$\\xi^\\star=\\frac{1}{2}$",
  "$\\xi^\\star=\\frac{3}{2}$",
  "$\\xi^\\star=3$",
  "+$\\xi^\\star=6$+",
