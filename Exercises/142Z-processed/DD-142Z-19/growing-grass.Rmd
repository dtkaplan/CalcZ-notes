...
askMC(
  ...
  "tons per day" = "These are the units of $C$ and therefore, because of the equality, of $\\partial_t G(t)$. We're asking about the units of $G(t)$, not $\\partial_t G(t)$.",
  "+tons+",
...
slice_plot(f(t) ~ t, domain(t=c(0,10))) %>% 
  gf_labs(x = "time", g = "grass biomass", title="Graph (A)")
slice_plot(pnorm(t, mean = 5, sd=2) ~ t, domain(t=c(0,10)) ) %>%
  gf_labs(x="time", y = "grass biomass", title = "Graph (B)")
slice_plot(f(g) ~ g, domain(g=c(0,10))) %>%
