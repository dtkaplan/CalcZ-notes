...
Note that $M^{2/3}$ is **not an allowed arithmetic operation**. $[M] = \mbox{mass}$, and mass, like any other dimension, cannot be raised to a non-integer power. More properly, the expression should be written $$\left(\frac{M}{1\  kg}\right)^{2/3}$$ The division by "1 kg" renders dimensionless the quantity in the parentheses: $$\left[\frac{M}{1\  kg}\right] = 1$$ 
...
$$S_{in}(M)= \frac{k}{2.6^2}\left(\frac{154\ \bcancel{\text{lbs}}}{1\ \cancel{\text{kg}}}\ \underbrace{\frac{1\ \cancel{\text{kg}}}{2.2\ \bcancel{\text{lbs}}}}_\text{flavor of 1}\right)^{2/3} = \underbrace{\frac{k}{2.6^2}\ \left(\frac{1}{2.2}\right)^{2/3}}_{k'\ \text{for inches and pounds}} 154^{2/3}$$
