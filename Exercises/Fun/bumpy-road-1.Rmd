...
slice_plot(road(x) ~ x, domain(x=c(-5,5))) %>%
  gf_labs(title="Plot (A)")
```

The graph shows a function that is a linear combination of three x-shifted hump functions
...
slice_plot(signal(x) ~ x, domain(x=c(-10,10))) %>%
...
Can you distinguish between the sexes in height?
