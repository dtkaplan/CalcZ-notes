...
slice_plot(dnorm(t, mean = 3, sd=1) ~ t,  domain(t = c(0,10))) %>%
  slice_plot(pnorm(t,  mean =3, sd = 1) ~ t) %>%
  gf_labs(x = "Time (minutes)")
```


```{r daily-digital-08-QA11, echo=FALSE}
askMC(
  prompt = "Which of these stories is consistent with the relationship between a hump and its corresponding sigmoid?",
