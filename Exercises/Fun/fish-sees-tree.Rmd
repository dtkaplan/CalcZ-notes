...
Each line that you pasted in the sandbox is a ***command***. The first command gives a value to $x$. The second command uses that value for $x$ to calculate a function output. The function is $g(x)\equiv \sin(x) \times \sqrt{\strut x}$. (You'll see how to give a function a name like $g()$ in Section \@ref(objects-and-actions).) 
