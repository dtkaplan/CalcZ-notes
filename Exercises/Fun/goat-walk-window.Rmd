...
slice_plot(drug(t) ~ t, domain(t=-2:10)) %>%
...
Let's explore a familiar-sounding directive: Take one pill every six hours. We can model this by a linear combination of time-shifted `drug()` functions. Each pill provides a dose of size `A`.

```{r}
six_hours <- 
  makeFun(A*drug(t, k) + A*drug(t-6, k) + 
          A*drug(t-12, k) + A*drug(t-18, k) +
          A*drug(t-24, k) ~ t, 
          k=log(2)/3, A  = 1)
