...
```{r echo=FALSE}
f <- rfun(~ t, seed=76)
slice_plot(f(t) ~ t, domain(t=c(-5,5))) %>%
  gf_refine(scale_x_continuous(breaks = -5:5),
            scale_y_continuous(breaks = 6:17)) %>%
  gf_text(6 ~ 5, label=~"B", color="red") %>%
  gf_text(6 ~ 3, label=~"A", color="red") %>%
...
say4 <- "It's \"rise\" over \"run\", not the other way around."
askMC(
...
slice_plot(f(t) ~ t, domain(t=c(-5,5))) %>%
  gf_refine(scale_x_continuous(breaks = -5:5),
            scale_y_continuous(breaks = 6:17)) %>%
  gf_text(6 ~ -3, label=~"B", color="red") %>%
  gf_text(6 ~ -5, label=~"A", color="red") %>%
...
askMC(
  ...
  "-17/2"=say1, "17/2"=say3, "2/17"=say4, "-2/17"=say4, "-10/2"=say3, "+10/2+", "2/10"=say3, "-2/10"=say4, "-17/-5" = say2
)
```
...
slice_plot(f(t) ~ t, domain(t=c(-5,5))) %>%
  gf_refine(scale_x_continuous(breaks = -5:5),
            scale_y_continuous(breaks = 6:17)) %>%
  gf_text(6 ~ -1.5, label=~"C", color="red") %>%
