...
Recall that each ***basic modeling functions*** are constructed from the corresponding naked modeling functions by scaling the input.

$$\text{naked modeling function}\ \ \underset{x\rightarrow a(x-x_0)}{\overset{\text{input scaling}}{\Large\Longrightarrow}} \ \ \text{basic modeling function}$$

Recognizing the axis crossings (y is blue, x is red) and asymptotes (orange) for the naked modeling functions can help you figure out the parameters $a$ and $x_0$ in the input scaling $a(x-x_0)$ for the basic modeling functions. Here's a reference: 
...
slice_plot(pnorm(x) ~ x, dom3) %>%
  ...
  gf_labs(title = "Naked sigmoid (pnorm())") %>%
...
slice_plot(dnorm(x) ~ x, dom1) %>%
  ...
  gf_labs(title = "Naked hump (dnorm())") %>%
  gf_lims(y=c(0,.5))
...
askMC(
  ...
  -2, "+-1+"= "Right. The location of the vertical asymtote is the clue.", 0, 1, "2" , random_answer_order = FALSE
)
```
