...
quadrants <- function(P) {
  P %>%
    gf_text(y ~ x, data = Pts, label=~label, color="red", group=NA,  alpha=0.4, size=20) %>%
    gf_hline(yintercept=0, color="red", size=3, alpha=0.4) %>%
    gf_vline(xintercept=0, color="red", size=3, alpha=0.4)
}
