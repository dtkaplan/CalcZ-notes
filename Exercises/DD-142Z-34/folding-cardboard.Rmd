---
title: "Folding cardboard"
author: "Daniel Kaplan"
---

```{r include=FALSE, setting="setup"}
library(etude2)
library(mosaic)
library(mosaicCalc)
```
...
knitr::include_graphics("www/folding-box.png")
```
...
Often, a constraint plays the role of a *dimension reduction*. With $w$, $h$, and $d$, we have a 3-input optimization problem. But we can use the constraint equation to solve for one of the variables as a function of the other variables and the (known) volume. For instance, we can find $h$ as
$$h = V/d w$$
...
When you draw the contour plot, you'll see a broad area near the center inside the contour at area = 9.5. Towards the upper-right and lower-left corners of the plot frame are contours at higher levels of area.
...
etude2::etudeQ(
  ...
  "All the points inside the 9.5 contour are at exactly the same height." = "The function $C(w,d)$ is infinitely differentiable so it's not possible to make a sudden shift from a sloped form to one that is dead flat."
)
```
...
etude2::etudeQ(
  "The following are values for the output of the function where you might be able to draw a contour. Which one of the values is the smallest for which a contour actually appears?",
...
etude2::etudeQ(
  "Since the end of the term is coming, here's a question that might be a good review for the final. Which of these describes the relationship between the gradient vector and the contours?",
...
etude2::etudeQ(
  ...
  "+The flap-length doesn't depend on $h$, only on $w d$. So we can make $h$ larger without contributing to the \"wasted\" area of the doubling over of flaps. The flaps get smaller as $wd$ gets smaller, so larger $h$ is preferred.+"
)
```
