...
f <- rfun(~ x + y, seed = 12426)
contour_plot(f(x, y) ~ x + y, domain(x=c(-5,5), y=c(-5,5)), filled=FALSE) %>%
  gf_segment(4 + 4 ~ -5 + 5, color = "black", size=2, alpha=.5) %>% #3
  gf_segment(-5 + 5 ~ 3 + -1, color = "orange", size=2, alpha=.5) %>% #
  gf_segment(-5 + 0 ~ -5 + 5, color = "red", size=2, alpha = .5) %>% #1
  gf_segment(-2 + -5  ~ -5 + 5, color = "yellow", size=2, alpha=.5) %>% # 4
...
slice_plot( f(-5 + 10*t, 4) ~ t, dom, label_text = "slice (3)", label_x=.5) %>% #black
  slice_plot( f(-5 + 10 *t, -5 + 5*t) ~ t, label_text = "slice (1)", label_x=.75) # red
