...
```{r echo=FALSE}
slice_plot(20000*pnorm(x, mean=30000/2, sd=30000/6) ~ x, domain(x=c(30000,0))) %>%
...
slice_plot(alt(t) ~ t, domain(t=c(0, 110)))
```
...
slice_plot(dd_alt(t) ~ t, domain(t=c(0,110)))
```
...
slice_plot(alt(t) ~ t, domain(t=c(-0, 200)))
dd_alt <- numD(alt(t) ~ t & t)
slice_plot(dd_alt(t) ~ t, domain(t=c(-0,200)))
```
