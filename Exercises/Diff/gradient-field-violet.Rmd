...
gradient_plot(f(x, y) ~ x + y, domain(x=c(-5,5), y=c(-5,5))) %>%
  gf_segment(4 + 4 ~ -5 + 5, color = "black", size=2, alpha=.5) %>% # 
  gf_segment(-5 + 5 ~ 3 + -1, color = "orange", size=2, alpha=.5) %>% #2
  gf_segment(-5 + 0 ~ -5 + 5, color = "red", size=2, alpha = .5) %>% #4
  gf_segment(-2 + -5  ~ -5 + 5, color = "yellow", size=2, alpha=.5) %>% # 3
