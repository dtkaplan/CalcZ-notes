...
```{r echo=FALSE}
f1 <- make_poly_0(f, -3)
slice_plot(f1(x) ~ x, domain(x=c(-5,5))) %>%
   gf_label(y ~ x0, data = Points, label=~label)  %>%
   gf_lims(y = c(-19, 0)) %>%
...
etude2::etudeQ(
  prompt = "What order approximation in graph (I)?",
    "+constant+" = "",
    "linear" = "A linear approximation would have exactly the same slope as $f()$ at the reference point $x_0$.",
    "quadratic" = "",
    "none of these" = "",
...
## Second graph
f2 <- make_poly_2(f, 0)
slice_plot(f2(x) ~ x, domain(x=c(-5,5))) %>%
   gf_label(y ~ x0, data = Points, label=~label)  %>%
   gf_lims(y = c(-19, 0)) %>%
...
etude2::etudeQ(
  ...
  "A" = "", "B" = "", "+C+" = "", 
  "D" = "At the reference position, the value of the approximation should always be $f(x_0)$. That's not the case here.",
  "E" = "", "None of them",
  random_answer_order = FALSE
)
```
...
slice_plot(f3(x) ~ x, domain(x=c(-5,5))) %>%
   gf_label(y ~ x0, data = Points, label=~label)  %>%
   gf_lims(y = c(-19, 0)) %>%
...
etude2::etudeQ(
  ...
  "A" = "", "B" = "", "C" = "", 
                 "D" = "At the reference position, the value of the approximation should always be $f(x_0)$. That's not the case here.", "E" = "", "+None of them+" = "It's not a polynomial approximation at any of  those points.",
  random_answer_order = FALSE
)
```
...
```{r echo=FALSE, warning = FALSE}
f4 <- make_poly_1(f, -4)
slice_plot(f4(x) ~ x, domain(x=c(-5,5))) %>%
   gf_label(y ~ x0, data = Points, label=~label)  %>%
   gf_lims(y = c(-19, 0)) %>%
...
```{r echo=FALSE, warning = FALSE}
f5 <- make_poly_2(f, -2)
slice_plot(f5(x) ~ x, domain(x=c(-5,5))) %>%
   gf_label(y ~ x0, data = Points, label=~label)  %>%
   gf_lims(y = c(-19, 0)) %>%
